...
func sum_array(array_len : felt, array : felt*) -> (sum : felt):
    ...
    let (current_sum) = sum_array(array_len=array_len - 1, array=array + 1)
    let sum = array[0] + current_sum
    return (sum=sum)
end
...
func max{range_check_ptr}(a : felt, b : felt) -> (max : felt):
    ...
    # Conditional jump
    # The following blocks are an assembly level equivalent of the if/else
    jmp b_is_more if [ap - 2] != 0  # here [ap-2] is the second value returne
